...
npm run provider -- monday            # regenerate this repo
npm run provider -- monday --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/monday-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/monday-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/monday-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-monday` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/monday/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/monday` |
| npm | `@particle-academy/monday-ui`, `@particle-academy/monday-js` |
| Packagist | `particle-academy/monday-php` |
| PyPI | `fancy-monday` |
...
**Default to plain HTTP.** A monday.com SDK is third-party code and is subject to
