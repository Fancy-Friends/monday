/*
 * monday.com — the published npm packages.
 *
...
import { mondayFaker } from "@particle-academy/monday-js";
import { MONDAY_KINDS } from "@particle-academy/monday-ui";
 ...
 */
const resolved = import.meta.resolve("@particle-academy/monday-js");
...
for (const { operation, config, expected } of GOLDENS) {
  const faked = mondayFaker(operation, fakeRequest("monday", operation, config));
...
assert.equal(MONDAY_KINDS.length, 1);
for (const kind of MONDAY_KINDS) {
...
console.log(`  ok   ui kinds resolve from ${"@particle-academy/monday-ui"}`);
